...
def plot_relative_angle_sensors(connected_sensors, sampling_frequency, sample_list, plot_name, first_sensor, second_sensor):

    lab = ["yaw", "pitch", "roll"]
    color = ["blue", "green", "cyan"]

    samples = len(sample_list[0][0])

    q = np.array(sample_list) # Convert list to np array
    
    qd = np.zeros(samples)   # Init result array
    euler_angles = np.zeros((3,samples)) # np array for plotting result
    ypr = np.zeros((3,1)) # np array to store result of quaternion - euler angles conversion

    for z in range(samples):
        q1 = pyq.Quaternion(q[(first_sensor-1),:,z] / 16384.0)   # convert to unity quaternions
        q2 = pyq.Quaternion(q[(second_sensor-1),:,z] / 16384.0)

        quat = q2.conjugate * q1   # Conjugate quaternions - calculate the angle in between

        [ypr[2], ypr[1], ypr[0]] = quaternion_to_euler_angle_vectorized(quat.w, quat.x, quat.y, quat.z) # Quaternion to euler angle - returns X Y Z

        euler_angles[0,z] = ypr[0] #*180/m.pi  # map euler angles to plotting variables
        euler_angles[1,z] = ypr[1] #*180/m.pi
        euler_angles[2,z] = ypr[2] #*180/m.pi

    fig = plt.figure(figsize=(18, 9))   # Setup figure plot
    x = np.linspace(0, samples / 1000, samples)
    plt.subplot(1,1,1)  # 1 subfigure
    plt.title('Angle between sensors %s and %s [%s Hz]' % (int(first_sensor), int(second_sensor), int(sampling_frequency)))
    for i in range(3):
        plt.plot(x, euler_angles[i,:], label=lab[i], color=color[i])
    plt.ylabel('Value [-]')
    plt.xlabel('Samples [KS]')
    plt.tight_layout()
    plt.legend()
